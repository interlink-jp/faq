...
私は[プログラミング未経験者とClaude Codeでグループウェアを開発する](https://note.interlink.blog/n/ncc90ca82c2e5)という取り組みを行いました。そこで気づいたのは、超初心者向けの解説が世の中にほとんどないということです。
...
Claude Codeを動かすために「Node.js」というソフトが必要です。

### Windowsの場合

[Node.js公式サイト](https://nodejs.org)を開き、**LTS版**のインストーラーをダウンロードして実行してください。設定はすべてそのまま（デフォルト）でOKです。

インストールが終わったら、**PowerShellを一度閉じて、もう一度開いてください**（ステップ2と同じ手順で）。

### Macの場合

以下のコマンドをターミナルにコピー＆ペーストして `Enter` を押してください。
...
以下のコマンドをコピー＆ペーストして `Enter` を押してください。バージョン番号（`v20.xx.x` のような数字）が表示されればOKです。
...
初回はブラウザが開いて、ログイン画面が表示されます。claude.aiのアカウントでログインしてください。ログイン後に「デスクトップでClaudeを最大限に活用」という画面が出ることがありますが、**「スキップ」を押してください**（デスクトップアプリは不要です）。ログインが完了すると、ターミナルに戻ります。
