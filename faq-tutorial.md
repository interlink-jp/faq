...
FAQシステムとは、「よくある質問と回答」を検索・表示できるWebアプリです。
...
まず、初回ログインのために以下のコマンドを実行します。

```
claude
```

ブラウザが開いて、ログイン画面が表示されます。claude.aiのアカウントでログインしてください。ログインが完了すると、ターミナルに戻ります。

`>` という入力欄が表示されたら、`/exit` と入力して一度終了します。
...
> **💡 ポイント：** このオプションをつけると、Claude Codeが途中で英語の確認メッセージを出さずに、最後まで自動で作業してくれます。練習用なので安全です。
...
ステップ6で `--dangerously-skip-permissions` をつけて起動していれば、途中で止まらず最後まで自動で進みます。
...
| Claude Codeが途中で止まった | `/exit` で一度終了して `claude --dangerously-skip-permissions` で再起動。「さっきの続きをやって」と入力 |
| 英語の確認メッセージが出て止まる | `claude --dangerously-skip-permissions` で起動し直してください（ステップ6参照） |
